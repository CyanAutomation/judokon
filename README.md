...
- `src/` – contains the game logic and assets:
  ...
  - `pages/`
    ...
    without moving the main image. The sidebar background uses `--color-surface`
    and items alternate with `--color-tertiary` for zebra striping defined in `sidebar.css`. It also includes `tooltipViewer.html`, a searchable viewer for
...
Default flag states live in `src/data/settings.json`, while labels and descriptions are pulled from `src/data/tooltips.json`. The **Tooltips** toggle on this page lets you turn help tooltips on or off globally.
