...
test("CLI skeleton and helpers smoke", async ({ page }) => {
  ...
  const focusHelper = await page.evaluate(
    () => !!window.__battleCLIinit && typeof window.__battleCLIinit.focusStats === "function"
  );
  if (focusHelper) {
    await page.evaluate(() => window.__battleCLIinit.focusStats());
    await expect(page.locator("#cli-stats")).toBeFocused();
    // Attempt to focus the next hint; some builds keep the hint non-focusable (aria-hidden).
    ...
    await expect(page.locator("#cli-stats")).toBeFocused();
    await page.focus("#cli-controls-hint").catch(() => {});
    const activeId = await page.evaluate(() => document.activeElement?.id || null);
    expect(activeId).not.toBeNull();
  }
});
