...
test.describe("Round Selection - Win Target Synchronization", () => {
  ...
  for (const { key, points, name } of testCases) {
    test(`should sync win target dropdown when ${name} is selected`, async ({ page }) => {
      ...
      await expect(roundCounter).toHaveText(new RegExp(`^Round \\d+ Target: ${points}$`));
    });
  }
  ...
  test("persists win target selection across reload", async ({ page }) => {
    ...
    await expect(page.locator("#round-counter")).toHaveText(/^Round \d+ Target: 10$/);
    ...
    try {
      ...
      await expect(revisit.locator("#round-counter")).toHaveText(/^Round \d+ Target: 10$/);
    } finally {
      await revisit.close();
    }
  });
