...
for (const vp of viewports) {
  ...
  const domIssues = [];
      ...
      const pageHasHorizontal = document.documentElement.scrollWidth > (window.innerWidth || document.documentElement.clientWidth);

      // find country flag container
      const flagTrack = document.querySelector(".country-flag-slide-track") || document.querySelector(".country-flag-slider") || document.querySelector("#country-list");
      ...
      const horizontalOverflows = nodes.filter(n => {
      ...
      const cardCandidates = nodes.filter(n => n.className && n.className.toString().toLowerCase().includes('card'));
      const verticalScrollCards = cardCandidates.filter(n => {
        try { return n.scrollHeight > n.clientHeight + 1; } catch(e) { return false; }
      }).slice(0,5).map(n => ({ tag: n.tagName.toLowerCase(), classes: n.className }));
...
for (const r of results) {
  console.log(`- ${r.viewport.name}: screenshot=${r.screenshot || 'n/a'}; error=${r.error || 'none'}`);
}
