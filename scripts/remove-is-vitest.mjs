...
async function transformSelectionHandler() {
  ...
  const forceDirectPattern =
    /const forceDirectResolution =\s+IS_VITEST && \(opts\.forceDirectResolution \|\| store\.forceDirectResolution\);/;
...
async function transformScoreboardView() {
  ...
  const isVitestBlock =
    /\s+const IS_VITEST = typeof process !== "undefined" && process\.env && process\.env\.VITEST;\s+if \(IS_VITEST\) \{\s+return;\s+\}/;
...
async function transformTestFiles() {
  ...
  for (const relPath of testFiles) {
    ...
    try {
      ...
      content = content.replace(
        /\s+if \(!process\.env\.VITEST\) \{\s+process\.env\.VITEST = ["']true["'];\s+\}/g,
        ""
      );

      // Remove console.log checking process.env.VITEST
      content = content.replace(
        /console\.log\(["']VITEST env at test top["'],\s*process\.env\.VITEST\);/g,
        ""
      );
