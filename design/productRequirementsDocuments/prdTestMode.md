...
| [`src/helpers/classicBattle/quitModal.js`](../../src/helpers/classicBattle/quitModal.js) | `quitConfirmButtonPromise` | Resolves when the quit confirmation modal renders the confirm button; tests can await this after opening the modal to avoid race conditions. | `await window.quitConfirmButtonPromise` (then interact with `#confirm-quit-button`). | `#confirm-quit-button` is rendered and ready for interaction. |
