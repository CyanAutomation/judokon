...
The former `design/stateHandlerAudit.md` compliance report is now captured here to keep implementation status aligned with the product requirements.
...
- **Important (Priority 2)**: Implement `announce:nextRoundInUI` (cooldown) to surface countdown updates through the snackbar / match header so players receive immediate feedback during cooldown; persist score/summary updates after each round; render the final match result screen; and ensure the match summary view renders on match end.
